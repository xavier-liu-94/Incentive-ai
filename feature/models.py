...
class CascadeSeqClassifier(torch.nn.Module):
    ...
    def __init__(self, num_class, sub_seq_len, pad_idx, dim_x=512) -> None:
        super().__init__()
        self.emb = ContinousSeqEmbedding(dim_x, 200, pad_idx)
    ...
    def forward(self, x, x_mask):
        ...
        if append_size > 0:
            x = torch.cat([x, self.pad_idx * torch.ones([batch, self.sub_seq_len-append_size])], dim=-1)
        ...
        x = x.reshape([batch * num_sub_seq, self.sub_seq_len])
        x_sub_mask = x_mask.reshape([batch * num_sub_seq, 1, self.sub_seq_len])

        # batch * num_sub_seq, sub_seq_len -> batch * num_sub_seq, sub_seq_len, dim_x
        seq_feature = self.emb(x)

        first_out = self.trans1(seq_feature, x_sub_mask)

        second_input = torch.mean(first_out.reshape([batch, num_sub_seq, self.sub_seq_len, -1]), dim=-2)
